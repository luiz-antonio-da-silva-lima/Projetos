...
int Buscar(char letra)
{
    if (isupper(letra))
    {
        ...
        int posiçao = pos - AlfabetoMaiusculo;
        return posiçao;
    }
    else
    {
        char *pos = strchr(Alfabeto, letra);
        int posiçao = pos - Alfabeto;
        return posiçao;
    }}
    char Criptografar(bool checkUpper, int posicaoinputalfabeto, int pular)
    {
        ...
        if (checkUpper)
        {
            return AlfabetoMaiusculo[(posicaoinputalfabeto + pular) % 26];
        }
        else
        {
            return Alfabeto[(posicaoinputalfabeto + pular) % 26];
        }
